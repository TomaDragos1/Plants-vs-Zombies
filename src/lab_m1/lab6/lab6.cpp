...
void Lab6::Init()
{
    ...
    // Create a shader program for drawing face polygon with the color of the normal
    {
        ...
        glUniform3fv(colorLoc, 1, glm::value_ptr(glm::vec3(1.0f, 1.0f, 1.0f))); // Culoare implicită albă
    }
}
...
void Lab6::Update(float deltaTimeSeconds)
{
    ...
    glm::vec3 positionVariation(
        sin(currentTime) * 2.0f,     // Variație pe axa X
        0.5f * cos(currentTime),     // Variație pe axa Y
        cos(currentTime) * 2.0f      // Variație pe axa Z
    );

    glm::vec3 colorVariation(
        0.5f + 0.5f * sin(currentTime),   // Variație pentru canalul roșu
        0.5f + 0.5f * cos(currentTime),   // Variație pentru canalul verde
        0.5f + 0.5f * sin(2 * currentTime) // Variație pentru canalul albastru
    );
    {
...

    {
        ...
        RenderSimpleMesh(meshes["cube"], shaders["VertexNormal"], modelMatrix, colorVariation, positionVariation);
    }

    {
        ...
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
        RenderSimpleMesh(meshes["cube"], shaders["LabShader"], modelMatrix, colorVariation, positionVariation);
    }
}
...
void Lab6::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color, const glm::vec3& poz)
{
    ...
    GLint PozLoc = glGetUniformLocation(shader->GetProgramID(), "Poz");
    glUniform1f(PozLoc, Engine::GetElapsedTime());
